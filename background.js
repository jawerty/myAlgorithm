...
const parseKeywordsFromText = (text) => {
  ...
  return namedEntities
    .map((entity) => {
      if (
        entity.text.indexOf('https://') === 0 ||
        entity.text.indexOf('https://') === 0
      ) {
        return ''
      }

      const parsedEntity = entity.text
        .replace(/[^\w\s]/gi, '')
        .replace(/\s+/g, ' ')
...
const processEngagementText = async (text, newEngagementType, sourceDomain) => {
  ...
  const keywordsToSave = keywords.map((keyword) => {
    ...
    return JSON.parse(
      JSON.stringify(
        new Keyword(pastKeywordObject, newEngagementType, sourceDomain)
      )
    )
  })
  ...
  try {
    ...
    const newRuntimeKeywords = Object.assign(
...
const init = () => {
  ...
  chrome.runtime.onMessage.addListener(function (
    ...
    if (request.action === 'myalgorithm-init') {
      ...
      if (
        Object.keys(runtimeObjects[storage.KEYS.custom_sources]).includes(
          customSourceData.domain
        )
      ) {
        sendResponse({
          success: false,
...
;(async function () {
  ...
  runtimeObjects[storage.KEYS.feed_settings] = await storage.get(
    storage.KEYS.feed_settings
  )
  runtimeObjects[storage.KEYS.fetched_ts] = await storage.get(
    storage.KEYS.fetched_ts
  )
  runtimeObjects[storage.KEYS.content_feed] = await storage.get(
    storage.KEYS.content_feed
  )
  runtimeObjects[storage.KEYS.custom_sources] = await storage.get(
    storage.KEYS.custom_sources
  )

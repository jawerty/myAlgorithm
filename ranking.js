...
function buildSearchQuery(topics, rankedKeywords, index, random) {
    ...
    if (random) {
        ...
        if (randomQueryType === 'single-random') {
            const randomTopic = rankedKeywords[Math.floor(Math.random()*rankedKeywords.length)];
            return randomTopic.text;

        } else if (randomQueryType === 'double-random') {
            const randomTopic1 = rankedKeywords[Math.floor(Math.random()*rankedKeywords.length)];
            let randomTopic2
            while((randomTopic2 != randomTopic1) && !randomTopic2) {
                randomTopic2 = rankedKeywords[Math.floor(Math.random()*rankedKeywords.length)];
            }

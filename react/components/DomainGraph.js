...
function DomainGraph({ keywords }) {
    ...
    useEffect(() => {
        if (keywords && chartContainer && chartContainer.current) {
          ...
          const labels = Object.keys(dataMap).sort((sourceDomainA, sourceDomainB) => {
    ...
    return <div className="domain-graph flex flex-column align-center justify-center">
        {keywords && keywords.length > 0 && <canvas ref={chartContainer} />}
        {!keywords || keywords.length === 0 && <div>No data to show</div>} 
        {keywords && keywords.length > 0 && <h2 className="domain-graph__title">Websites you engage with the most (by occurrence)</h2>}
    </div>
}

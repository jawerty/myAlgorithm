...
function TopicEditor({ getKeywords, keywords, setKeywords }) {
  ...
  return (
    <div className="topic-editor flex flex-column align-start">
      <div className="topic-editor flex flex-column align-start justify-start">
        <span className="mb-1 topic-editor__title">
          Edit your algorithm: (topics are generated from your browsing habits)
        </span>

        {keywords && keywords.length > 0 && (
          <div className="topic-editor flex flex-wrap align-center">
            {keywords.slice(0, showCount).map((keyword, i) => {
              return (
                ...
                >
                  <div className="topic-editor__topic-tag flex align-center">
                    {keyword.text}{' '}
                    <span
                      className="topic-editor_x"
                      onClick={() => removeKeyword(keyword.text, i)}
                    >
                      X
                    </span>
                  </div>
        ...
        <div
          className={`topic-editor__forms flex${
            showAddTopicForm ? ' flex-column' : ''
          } align-start`}
        >
          {keywords && keywords.length > 0 && (
            <div
          ...
          {showAddTopicForm && (
            <div className="topic-editor__add-topic-form flex align-center">
              <input
                value={newTopic}
                onChange={(e) => {
              ...
              />
              <div
                className="topic-editor__add-topic-submit"
                onClick={() => addTopic()}
              >
                Add Topic
              </div>

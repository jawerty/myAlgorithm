...
function ContentFeed({}) {
    ...
    return new Promise((resolve) => {
      chrome.runtime.sendMessage(
        ...
        async (response) => {
          ...
          const newContentFeed = await ContentFetcher(
            feedSettings.sourcing,
            response.customSources,
  ...
  return (
    <div className="content-feed flex flex-column">
      {loading && (
        <div className="content-feed__loading-view full-width flex align-center flex-column justify-center">
          <img src={loadingSvg} />
          <span>Please wait for your daily feed to load (~10-15 seconds)</span>
        </div>
      )}

      {!loading && feedSettings?.refreshMode && (
        <div
          className="content-feed__refresh"
          onClick={async () => {
            setLoading(true)
            const newContentFeed = await getNewContentFeed()
            console.log(newContentFeed)
      ...
      {!loading && contentFeed && contentFeed.length > 0 && (
        <div className="full-width flex align-center flex-column justify-center">
      ...
      {!loading && (!contentFeed || contentFeed.length === 0) && (
        <div className="full-width flex align-center flex-column justify-center">
          {keywords && keywords.length > 0 && (
            <span>Could not find content</span>
          )}
          {(!keywords || keywords.length === 0) && (
            <div>No data yet :( start browsing and build up your algorithm</div>
          )}
        </div>
      )}
    </div>

    ...
    fetch(link, {
      ...
      .then(function (text) {
        ...
        if (metaOGTitle) {
          ...
          const metaTwitterTitle = doc.querySelector(
        ...
        if (metaOGDesc) {
          ...
          const metaTwitterDesc = doc.querySelector(
        ...
        if (metaOGImage) {
          ...
          const metaTwitterImage = doc.querySelector(
...
async function YandexFetcher(searchQuery, site) {
  ...
  return new Promise((resolve) => {
    ...
    const searchUrl = `https://yandex.com/search/?text=${encodeURIComponent(
      searchQuery
    )}+site:${site}+lang:en&lr=103017&redircnt=1657655444.1`
    fetch(searchUrl, {
      method: 'GET',
      headers: {
        accept:
      ...
      .then(function (text) {
        ...
        const links = doc.querySelectorAll(
          '.main__content .serp-list .serp-item .OrganicTitle-Link'
        )
        console.log(links)
...
async function ContentFetcher(sourcingSettings, customSources, searchQueries) {
  ...
  let sitesToSource = Object.keys(sourcingSettings)
    .filter((source) => {
  ...
  if (Object.keys(customSources).length > 0) {
    const enabledCustomSources = Object.values(customSources)
      .filter((customSource) => {
  ...
  for (let searchQuery of searchQueries) {
    ...
    const fetchers = [
      ...
      GoogleFetcher,
      GoogleFetcher,
    ] // randomness for the fetcher (avoid rate limiting)
    let fetcher
...
export { ContentFetcher, fetchContentSourceDetails }
